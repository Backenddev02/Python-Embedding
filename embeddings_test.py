...
QDRANT_URL = os.getenv(
    "QDRANT_URL",
    "https://558d3fea-5962-46da-bffa-94aba210a6c6.eu-west-1-0.aws.cloud.qdrant.io",
)
QDRANT_API_KEY = os.getenv("QDRANT_API_KEY", "REPLACE_ME_WITH_REAL_KEY")
...
def ensure_payload_index(
    ...
    field_schema can be:
      - PayloadSchemaType.KEYWORD / INTEGER / FLOAT / DATETIME / TEXT / UUID / GEO
      - TextIndexParams(...) for advanced full-text config
      - string like "keyword" / "integer" etc.
    """
    try:
        ...
        msg = str(e).lower()
        # Qdrant returns an error if index already exists; we can safely ignore that
        if "already exists" in msg or "index for field" in msg:
            ...
            # you can choose to raise here if you want hard failure
            # raise


def ensure_segments_indexes():
    """
    Create all useful indexes on SEGMENTS_COLLECTION so that
    every important field is searchable / filterable.
    """
    # numeric fields
    ensure_payload_index(
        SEGMENTS_COLLECTION, "video_id", PayloadSchemaType.INTEGER
    )
    ensure_payload_index(
        SEGMENTS_COLLECTION, "segment_index", PayloadSchemaType.INTEGER
    )
    ensure_payload_index(
        SEGMENTS_COLLECTION, "text_length", PayloadSchemaType.INTEGER
    )

    ensure_payload_index(
        SEGMENTS_COLLECTION, "start_time", PayloadSchemaType.FLOAT
    )
    ensure_payload_index(
        SEGMENTS_COLLECTION, "end_time", PayloadSchemaType.FLOAT
    )
    ensure_payload_index(
        SEGMENTS_COLLECTION, "duration", PayloadSchemaType.FLOAT
    )
    ensure_payload_index(
        SEGMENTS_COLLECTION, "confidence", PayloadSchemaType.FLOAT
    )

    # datetime
    ensure_payload_index(
        SEGMENTS_COLLECTION, "created_at", PayloadSchemaType.DATETIME
    )

    # keyword/string fields
    ensure_payload_index(
        SEGMENTS_COLLECTION, "video_title", PayloadSchemaType.KEYWORD
    )
    ensure_payload_index(
        SEGMENTS_COLLECTION, "video_filename", PayloadSchemaType.KEYWORD
    )
    ensure_payload_index(
        SEGMENTS_COLLECTION, "youtube_url", PayloadSchemaType.KEYWORD
    )
    ensure_payload_index(
        SEGMENTS_COLLECTION, "language", PayloadSchemaType.KEYWORD
    )
    ensure_payload_index(
        SEGMENTS_COLLECTION, "speaker", PayloadSchemaType.KEYWORD
    )
    ensure_payload_index(
        SEGMENTS_COLLECTION,
        "diarization_speaker",
    ...
    ensure_payload_index(
        SEGMENTS_COLLECTION, "match_type", PayloadSchemaType.KEYWORD
    )

    # full-text on transcript text
    ensure_payload_index(
        SEGMENTS_COLLECTION,
        "text",
        TextIndexParams(
...
def ensure_legacy_indexes():
    ...
    """
    ensure_payload_index(
        LEGACY_COLLECTION, "video_id", PayloadSchemaType.INTEGER
    )
    ensure_payload_index(
        LEGACY_COLLECTION, "text_length", PayloadSchemaType.INTEGER
    )
    ensure_payload_index(
        LEGACY_COLLECTION, "created_at", PayloadSchemaType.DATETIME
    )
    ensure_payload_index(
        LEGACY_COLLECTION, "source", PayloadSchemaType.KEYWORD
    )
    ensure_payload_index(
        LEGACY_COLLECTION,
        "text",
...
def create_segments_collection():
    try:
        exists = qdrant_client.collection_exists(collection_name=SEGMENTS_COLLECTION)

        if not exists:
            print(
                f"Creating collection '{SEGMENTS_COLLECTION}'...", flush=True
            )
            ...
            print(
                f"Collection '{SEGMENTS_COLLECTION}' created successfully",
                flush=True,
            )
        else:
            print(
                f"Collection '{SEGMENTS_COLLECTION}' already exists",
                flush=True,
            )

        # Ensure all indexes exist (safe to call even if collection pre-exists)
...
def create_legacy_collection():
    try:
        exists = qdrant_client.collection_exists(collection_name=LEGACY_COLLECTION)

        if not exists:
            print(
                f"Creating legacy collection '{LEGACY_COLLECTION}'...",
                flush=True,
            )
            qdrant_client.create_collection(
            ...
            print(
                f"Collection '{LEGACY_COLLECTION}' created successfully",
                flush=True,
            )
        else:
            print(
                f"Legacy collection '{LEGACY_COLLECTION}' already exists",
                flush=True,
            )
...
def delete_existing_embeddings(video_id: int):
    try:
        ...
        print(
            f"Deleted existing embeddings for video {video_id}", flush=True
        )
    except Exception as e:
        print(
            f"Note: Could not delete embeddings (may not exist): {str(e)}",
            flush=True,
        )
...
async def embed_video(data: dict):
    try:
        ...
        if not identification_segments:
            raise HTTPException(
                status_code=400,
                detail="identification_segments is required",
            )
        ...
        print(
            f"Processing video {video_id} with {len(identification_segments)} segments",
            flush=True,
        )

        # clear old embeddings for this video_id
        ...
        if not texts_to_embed:
            raise HTTPException(
                status_code=400,
                detail=(
                    "No valid segments found to embed.  "
                    f"Total: {len(identification_segments)}, "
                    f"Without text: {segments_without_text}"
                ),
            )

        print(
            f"Generating embeddings for {len(texts_to_embed)} segments in batch...",
            flush=True,
        )
        batch_start_time = datetime.utcnow()
        vectors = model.encode(
            texts_to_embed, show_progress_bar=False, batch_size=32
        ).tolist()
        batch_end_time = datetime.utcnow()
        batch_duration = (batch_end_time - batch_start_time).total_seconds()
        print(
            f"Batch embedding completed in {batch_duration:.2f} seconds",
            flush=True,
        )
        ...
        try:
            ...
            print(
                f"Successfully inserted {len(points)} points", flush=True
            )
        except Exception as e:
            print(f"ERROR: Qdrant insertion failed: {str(e)}", flush=True)
            raise HTTPException(
                status_code=500, detail=f"Qdrant error: {str(e)}"
            )
...
async def embed(data: dict):
    ...
    vector_id = (
        f"video_{video_id}_{uuid.uuid4()}" if video_id else str(uuid.uuid4())
    )
...
async def search(data: dict):
    ...
    if not query_text and not word:
        raise HTTPException(
            status_code=400, detail="Either 'query' or 'word' is required"
        )
    ...
    if video_id_filter is not None:
        filter_conditions.append(
            FieldCondition(
                key="video_id", match=MatchValue(value=video_id_filter)
            )
        )
    if speaker_filter is not None:
        filter_conditions.append(
            FieldCondition(
                key="speaker", match=MatchValue(value=speaker_filter)
            )
        )
    ...
    if query_text:
        try:
            print(
                f"Semantic search for: '{query_text[:120]}'", flush=True
            )
                ...
                semantic_results.append(
                    {
                        ...
                        "video_title": r.payload.get(
                            "video_title", ""
                        ),
                        "speaker": r.payload.get("speaker", ""),
                        "diarization_speaker": r.payload.get(
                            "diarization_speaker", ""
                        ),
                        ...
                        "duration": round(
                            (
                                r.payload.get("end_time", 0)
                                - r.payload.get("start_time", 0)
                            ),
                            2,
                        ),
                        "text": r.payload.get("text", ""),
                        "text_length": r.payload.get(
                            "text_length", 0
                        ),
                        "youtube_url": r.payload.get(
                            "youtube_url", ""
                        ),
            ...
            print(
                f"ERROR during semantic search: {str(e)}", flush=True
            )
            raise HTTPException(
                status_code=500,
    ...
    if word:
        try:
            print(
                f"Keyword search for word: '{word}' (max_scanned={max_scanned})",
                flush=True,
            )
                        ...
                        keyword_results.append(
                            {
                                "id": p.id,
                                "score": 1.0,
                                "video_id": p.payload.get(
                                    "video_id"
                                ),
                                "video_title": p.payload.get(
                                    "video_title", ""
                                ),
                                "speaker": p.payload.get(
                                    "speaker", ""
                                ),
                                "diarization_speaker": p.payload.get(
                                    "diarization_speaker", ""
                                ),
                                "start_time": p.payload.get(
                                    "start_time", 0
                                ),
                                "end_time": p.payload.get(
                                    "end_time", 0
                                ),
                                "duration": round(
                                    (
                                ...
                                "text": text_val,
                                "text_length": p.payload.get(
                                    "text_length", 0
                                ),
                                "youtube_url": p.payload.get(
                                    "youtube_url", ""
                                ),
                                "language": p.payload.get(
                                    "language", ""
                                ),
            ...
            print(
                f"ERROR during keyword search: {str(e)}", flush=True
            )
            raise HTTPException(
                status_code=500,
...
async def search_multi_video(data: dict):
    ...
    if not video_ids:
        raise HTTPException(
            status_code=400, detail="video_ids list is required"
        )

    print(
        f"Searching across {len(video_ids)} videos for: '{query_text[:100]}'",
        flush=True,
    )
                ...
                results_by_video[vid].append(
                    {
                        ...
                        "text": r.payload.get("text", ""),
                        "youtube_url_timestamped": (
                            f"{r.payload.get('youtube_url', '')}"
                            f"?t={int(r.payload.get('start_time', 0))}"
...
async def get_video_segments(
    video_id: int, limit: int = 100, offset: int = 0
):
    try:
        points, next_offset = qdrant_client.scroll(
            collection_name=SEGMENTS_COLLECTION,
...
if __name__ == "__main__":
    ...
    print(
        f"Starting FastAPI Video Embedding Service on port {port}...",
        flush=True,
    )
    print(f"Qdrant URL: {QDRANT_URL}", flush=True)
    print(
        f"Collections: {SEGMENTS_COLLECTION}, {LEGACY_COLLECTION}",
        flush=True,
    )
